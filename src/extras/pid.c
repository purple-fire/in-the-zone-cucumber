...
void pidDataInit(PIDData *data, float kp, float ki, float kd, float maxPower,
...
float pidNextIteration(PIDData *data, float error) {
  ...
  /*
          if ((error * data->kp < data->maxPower) && (error * data->kp >
     -(data->maxPower))) {
  ...
  */
  ...
  result = ((data->kp * proportional) + (data->ki * integral) +
            (data->kd * derivative));
...
result = (data->kp * proportional) + (data->ki * integral) + (data->kd *
derivative);
